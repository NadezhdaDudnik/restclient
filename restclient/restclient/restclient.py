...
def allure_attach(fn):
    def wrapper(*args, **kwargs):
        ...
        if body:
            allure.attach(json.dumps(kwargs.get('json'), indent=2), name='request',
        ...
        try:
            ...
            allure.attach(response_text if len(response_text) > 0 else status_code, name='response',
                          attachment_type=allure.attachment_type.TEXT)
        else:
            allure.attach(json.dumps(response_json, indent=2), name='response',
                          attachment_type=allure.attachment_type.JSON)
        return response
